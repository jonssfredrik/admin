...
import { notificationChannels, notificationRules, siteName } from "@/modules/jetwp/data";
                ...
                <div key={rule.id} className="px-5 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {rule.eventTypes.map((eventType) => (
                          <Badge key={eventType} tone="warning">
                            {eventType}
                          </Badge>
                        ))}
                      </div>
                  ...
                  {isExpanded && (
                    <div className={clsx("mt-3 space-y-1 rounded-lg border bg-bg/40 p-3")}>
                      ...
                      {rule.siteIds.map((id) => (
                        <Link key={id} href={`/jetwp/${id}`} className="flex items-center gap-2 rounded-md px-2 py-1 text-xs hover:bg-bg">
