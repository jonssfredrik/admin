...
import { nodeThresholdAlerts, sites } from "@/modules/jetwp/data";
      ...
      <Card className="overflow-hidden p-0">
        <div className="flex items-center justify-between border-b px-5 py-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted">{nodes.length} noder · {nodes.reduce((sum, node) => sum + node.sites, 0)} sajter totalt</p>
          </div>
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2">
            {sites.map((site) => {
              ...
              return (
                <Link key={site.id} href={`/jetwp/${site.id}`} className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm transition-colors hover:bg-bg">
