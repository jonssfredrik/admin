...
const registrars: Registrar[] = [
  {
    ...
    name: "Loopia",
    country: "se",
    url: (domain) => `https://www.loopia.se/domain/?domain=${encodeURIComponent(domain)}`,
  ...
  },
  {
    ...
    name: "Inleed",
    country: "se",
    url: (domain) => `https://www.inleed.se/webshop?domain=${encodeURIComponent(domain)}`,
  ...
  },
  {
    ...
    name: "One.com",
    country: "global",
    url: (domain) => `https://www.one.com/en/domain?domain=${encodeURIComponent(domain)}`,
...
export function RegisterActions({ domain }: { domain: DomainAnalysis }) {
  const preferred = domain.tld === ".se" || domain.tld === ".nu"
    ? registrars.filter((registrar) => registrar.country === "se")
    : registrars.filter((registrar) => registrar.country === "global");
  const rest = registrars.filter((registrar) => !preferred.includes(registrar));
  ...
  return (
    <Card className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {preferred.map((registrar) => (
          <RegistrarLink key={registrar.id} registrar={registrar} domain={domain.domain} primary />
        ))}
        {rest.map((registrar) => (
          <RegistrarLink key={registrar.id} registrar={registrar} domain={domain.domain} />
        ))}
      </div>
    </Card>
  );
...
function RegistrarLink({ registrar, domain, primary }: { registrar: Registrar; domain: string; primary?: boolean }) {
  return (
    <a
    ...
    >
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-surface text-xs font-semibold">
        {registrar.name.slice(0, 2).toUpperCase()}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="truncate text-sm font-semibold">{registrar.name}</span>
          {primary && <Badge tone="neutral">Rekommenderad</Badge>}
        </div>
        <div className="text-xs text-muted">{registrar.estimate}</div>
      </div>
