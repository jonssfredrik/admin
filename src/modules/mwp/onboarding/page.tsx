...
import { Check, Copy, Server, Key, Terminal, Activity, Shield, RefreshCw } from "lucide-react";
...
export default function OnboardingPage() {
  ...
  const install = `curl -sSL https://mwp.io/install.sh | sudo bash -s -- \\
  --token <TOKEN> \\
  --server ${server || "<server-id>"} \\
  --path ${path}`;

  const revoke = (token: string) => {
    setTokens((prev) => prev.map((entry) => entry.token === token ? { ...entry, used: true, expiresAt: "återkallad" } : entry));
  ...
  return (
    <div className="space-y-6">
      <MWPPageIntro title="Onboarding" subtitle="Parkoppla nya servrar och WordPress-installationer med MWP" />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-sm font-semibold">3. Vänta på incheckning</div>
      ...
      <Card className="overflow-hidden p-0">
        ...
        <div className="border-t px-5 py-3 text-[11px] text-muted">
          Kör som root på målservern. Agenten verifierar token mot kontrollplanet och registrerar automatiskt sajten vid första incheckningen.
        </div>
      </Card>
              ...
              <div key={t.token} className="flex items-center gap-3 px-5 py-3">
                ...
                {!t.used && (
                  <div className="flex gap-2">
                    ...
                    <Button variant="secondary" onClick={() => extend(t.token)}>Förläng</Button>
                    <Button variant="secondary" onClick={() => revoke(t.token)}>Återkalla</Button>
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="divide-y divide-border/60">
            {authEvents.map((e, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-2.5 text-sm">
                ...
                <Link href={`/mwp/${e.siteId}`} className="flex-1 truncate text-[12px] hover:text-fg">{e.server}</Link>
      ...
      <Card className="p-5">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <div className="rounded-lg border bg-bg/40 p-3">
            <div className="text-xs font-medium uppercase tracking-wider text-muted">Incheckning</div>
            <div className="mt-2 text-sm">Senaste incheckning inom 30 sek</div>
          </div>
          <div className="rounded-lg border bg-bg/40 p-3">
            <div className="text-xs font-medium uppercase tracking-wider text-muted">Integritet</div>
            <div className="mt-2 text-sm">Kör integrity.check på sajten</div>
          </div>
          <div className="rounded-lg border bg-bg/40 p-3">
            <div className="text-xs font-medium uppercase tracking-wider text-muted">Baseline</div>
            <div className="mt-2 text-sm">Skapa första säkerhetskopian och koppla notifieringsgrupp</div>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <ul className="mt-3 space-y-1.5 text-sm text-muted">
          <li>• Verifiera att incheckningar kommer in (max 30 sek).</li>
          <li>• Kör <span className="font-mono text-[12px]">integrity.check</span> för att skanna kärnfiler.</li>
          <li>• Skapa en första baseline-säkerhetskopia.</li>
          <li>• Aktivera relevanta arbetsflöden (säkerhetsskanning, automatiska uppdateringar).</li>
