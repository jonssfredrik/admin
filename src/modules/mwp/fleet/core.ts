import { sites } from "@/modules/mwp/data/core";
...
export const jobs: Job[] = [
  ...
  },
  {
    ...
    workflow: { id: "security-scan", name: "Sakerhetsskanning varje natt", step: 2 },
  },
  {
    id: "job-1043",
    siteId: "site-5",
...
export const jobTypes: { value: JobType; label: string; description: string; paramFields?: { name: string; label: string; placeholder?: string; kind?: "text" | "select"; options?: string[] }[] }[] = [
  ...
  { value: "ssl.renew", label: "Fornya SSL", description: "Kor Let's Encrypt-fornyelse" },
  { value: "agent.upgrade", label: "Uppgradera agent", description: "Uppgradera mwp-agent pa sajten", paramFields: [{ name: "targetVersion", label: "Malversion", placeholder: "0.8.2" }] },
];
...
export const alerts: Alert[] = [
  { id: "al-1", kind: "missed_heartbeat", severity: "critical", status: "open", siteId: "site-5", title: "Missad heartbeat - arcticoutdoor.com", description: "Agenten har inte rapporterat in sedan 13:38. Forvantas var 30:e sekund.", createdAt: "for 42 min sedan", source: "mwp-agent" },
  { id: "al-2", kind: "failed_job", severity: "critical", status: "open", siteId: "site-5", title: "Plugin-uppdatering misslyckades (3/3 forsok)", description: "job-1045 · elementor -> fatal error efter update. Rollback korde automatiskt.", createdAt: "for 15 min sedan", source: "jobs" },
  { id: "al-3", kind: "ssl_expiring", severity: "warning", status: "open", siteId: "site-2", title: "SSL gar ut om 12 dagar", description: "lagominterior.se · certbot-fornyelse fastnade i DNS-verifiering.", createdAt: "for 3 tim sedan", source: "monitoring" },
  { id: "al-4", kind: "security_update", severity: "warning", status: "open", siteId: "site-2", title: "Kritisk sakerhetsuppdatering tillganglig", description: "Wordfence 7.11.6 innehaller fix for CVE-2026-1042. 2 sajter paverkade.", createdAt: "for 5 tim sedan", source: "inventory" },
  { id: "al-5", kind: "stale_telemetry", severity: "warning", status: "acked", siteId: "site-5", title: "Stale telemetry > 30 min", description: "Senaste telemetri fran arcticoutdoor.com ar 42 min gammal.", createdAt: "for 1 tim sedan", source: "mwp-agent" },
  { id: "al-6", kind: "disk_space", severity: "warning", status: "open", siteId: "site-2", title: "Disk > 90% full", description: "lagominterior.se · 9.1/10 GB i bruk. Overvag att rensa media eller uppgradera plan.", createdAt: "for 2 tim sedan", source: "monitoring" },
  { id: "al-7", kind: "failed_job", severity: "info", status: "resolved", siteId: "site-5", title: "SSL-fornyelse avbruten", description: "job-1037 avbrots manuellt av Greta T.", createdAt: "for 4 tim sedan", source: "jobs" },
  { id: "al-8", kind: "missed_heartbeat", severity: "info", status: "resolved", siteId: "site-4", title: "Heartbeat aterupptagen", description: "fjallkliniken.se ar tillbaka efter planerat underhall.", createdAt: "igar", source: "mwp-agent" },
];
