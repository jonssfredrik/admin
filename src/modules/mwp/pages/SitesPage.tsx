...
import { jobsForSite, sites, type Plan, type SiteRecord, type SiteStatus } from "@/modules/mwp/data";
import { getFilteredSites, getSiteListStats, type SiteSortDir, type SiteSortKey } from "@/modules/mwp/selectors/sites";
...
export function SitesPage() {
  ...
  const menuFor = (site: SiteRecord): RowMenuEntry[] => [
    { label: "Öppna detaljer", icon: ExternalLink, onClick: () => router.push(`/mwp/${site.id}`) },
    ...
    { divider: true },
    { label: "Nytt jobb", icon: Play, onClick: () => router.push(`/mwp/jobs/new?site=${site.id}`) },
    { divider: true },
    {
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-3 flex items-start justify-between gap-4">
          <PageHeader title="Sajter" subtitle="Hela flottan av WordPress-sajter som hanteras av MWP-agenten" />
            ...
            {paged.map((site) => {
              ...
              return (
                <tr key={site.id} className={clsx("group transition-colors hover:bg-bg/60", selected[site.id] && "bg-bg")}>
                  ...
                  <td className="border-b border-border/60 px-4 py-3">
                    <Link href={`/mwp/${site.id}`} className="flex items-center gap-3">
