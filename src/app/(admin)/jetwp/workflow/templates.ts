...
const onboarding: WorkflowTemplate = {
  ...
  nodes: [
    { id: "n1", kind: "trigger", icon: Zap, title: "Ny sajt skapad", subtitle: "JetWP · webhook-event", x: 40, y: 220,
      config: [{ label: "Event", value: "site.created" }, { label: "Filter", value: "plan ≥ Business" }] },
    { id: "n2", kind: "tool", icon: Plug, title: "Installera baspaket", subtitle: "Yoast, WP Rocket, Wordfence", x: 340, y: 90,
      config: [{ label: "Plugins", value: "3 st" }, { label: "Tema", value: "Kadence" }] },
    { id: "n3", kind: "agent", icon: Sparkles, title: "SEO-analys", subtitle: "GPT-4o genererar metabeskrivningar", x: 340, y: 350,
      config: [{ label: "Modell", value: "gpt-4o" }, { label: "Max tokens", value: "1024" }] },
    { id: "n4", kind: "logic", icon: GitBranch, title: "Uppdateringar tillgängliga?", subtitle: "If / else", x: 660, y: 220,
      config: [{ label: "Uttryck", value: "site.updatesAvailable > 0" }] },
    { id: "n5", kind: "guardrail", icon: Shield, title: "Kräv färsk backup", subtitle: "Max 24h gammal", x: 980, y: 90,
      config: [{ label: "Max ålder", value: "24h" }] },
    { id: "n6", kind: "tool", icon: RotateCw, title: "Kör uppdateringar", subtitle: "wp-cli unattended", x: 1280, y: 90,
      config: [{ label: "Timeout", value: "5 min" }] },
    { id: "n7", kind: "output", icon: Bell, title: "Välkomstmail + Slack", subtitle: "Notifiera ägare och ops-team", x: 1280, y: 350,
...
const securityScan: WorkflowTemplate = {
  ...
  nodes: [
    { id: "n1", kind: "trigger", icon: Clock, title: "Varje natt 02:00", subtitle: "Cron · 0 2 * * *", x: 40, y: 220,
      config: [{ label: "Tidzon", value: "Europe/Stockholm" }] },
    { id: "n2", kind: "tool", icon: Shield, title: "Wordfence-scan", subtitle: "Alla sajter · full scan", x: 340, y: 220,
      config: [{ label: "Omfattning", value: "filer + db" }, { label: "Timeout", value: "30 min" }] },
    { id: "n3", kind: "agent", icon: Sparkles, title: "Klassificera fynd", subtitle: "GPT-4o · severity + åtgärd", x: 660, y: 220,
      config: [{ label: "Modell", value: "gpt-4o" }] },
    { id: "n4", kind: "logic", icon: GitBranch, title: "Kritisk nivå?", subtitle: "If severity ≥ hög", x: 980, y: 220,
      config: [{ label: "Uttryck", value: "severity in [hög, kritisk]" }] },
    { id: "n5", kind: "output", icon: Bell, title: "Slack-larm + e-post", subtitle: "#security-alerts + ägare", x: 1280, y: 220,
...
const pluginUpdates: WorkflowTemplate = {
  ...
  nodes: [
    { id: "n1", kind: "trigger", icon: Clock, title: "Söndagar 03:00", subtitle: "Cron · 0 3 * * 0", x: 40, y: 220,
    ...
    { id: "n3", kind: "guardrail", icon: Shield, title: "Skapa backup", subtitle: "Full snapshot innan ändring", x: 640, y: 220,
    ...
    { id: "n5", kind: "tool", icon: Globe, title: "Smoke test", subtitle: "HTTP 200 på hemsida + admin", x: 1240, y: 220,
      config: [{ label: "Endpoints", value: "/, /wp-admin" }] },
    { id: "n6", kind: "logic", icon: GitBranch, title: "Allt OK?", subtitle: "If status === 200", x: 1540, y: 220 },
    { id: "n7", kind: "tool", icon: RotateCw, title: "Rollback", subtitle: "Återställ från backup", x: 1840, y: 90,
...
const contentTriage: WorkflowTemplate = {
  ...
  nodes: [
    ...
    { id: "n2", kind: "guardrail", icon: EyeOff, title: "Maskera PII", subtitle: "Dölj e-post + telefon i loggar", x: 340, y: 220,
      config: [{ label: "Fält", value: "email, phone" }] },
    { id: "n3", kind: "agent", icon: Sparkles, title: "Klassificera inlägg", subtitle: "GPT-4o · sälj / support / spam", x: 640, y: 220,
    ...
    { id: "n5", kind: "tool", icon: Globe, title: "Dirigera till rätt system", subtitle: "CRM (sälj) · Helpdesk (support)", x: 1240, y: 220,
...
const sslRenewal: WorkflowTemplate = {
  ...
  nodes: [
    ...
    { id: "n2", kind: "tool", icon: Globe, title: "Hämta SSL-status", subtitle: "För alla sajter i JetWP", x: 340, y: 220,
    ...
    { id: "n4", kind: "output", icon: Mail, title: "Skicka påminnelse", subtitle: "E-post till sajtägare", x: 940, y: 220,
...
const uptimeAlert: WorkflowTemplate = {
  ...
  nodes: [
    ...
    { id: "n2", kind: "tool", icon: Globe, title: "Pinga alla sajter", subtitle: "HTTP HEAD · timeout 5s", x: 340, y: 220,
    ...
    { id: "n4", kind: "guardrail", icon: Shield, title: "Dedupe larm", subtitle: "Max 1 larm/sajt per 5 min", x: 940, y: 220,
