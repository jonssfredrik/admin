...
const kindStyles: Record<NodeKind, { ring: string; chip: string; dot: string; label: string }> = {
  trigger: { ring: "border-blue-500/40", chip: "bg-blue-500/10 text-blue-600 dark:text-blue-400", dot: "bg-blue-500", label: "Trigger" },
  ...
  guardrail: { ring: "border-rose-500/40", chip: "bg-rose-500/10 text-rose-600 dark:text-rose-400", dot: "bg-rose-500", label: "Guardrail" },
  output: { ring: "border-slate-500/40", chip: "bg-slate-500/10 text-slate-600 dark:text-slate-400", dot: "bg-slate-500", label: "Output" },
};
...
const palette: { group: string; items: PaletteItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "agent", icon: MessageSquare, title: "Custom prompt", subtitle: "Instruktion" },
    ],
  },
  {
    group: "Logik",
    ...
    <div className="flex h-[calc(100vh-4rem)] min-h-[600px] flex-col -m-6 sm:-m-8">
      <div className="flex items-center justify-between gap-4 border-b bg-surface px-5 py-2.5">
        <div className="flex items-center gap-3">
          <Link href="/jetwp/workflow" className="flex h-7 w-7 items-center justify-center rounded-md border bg-surface text-muted hover:text-fg">
            <ArrowLeft size={14} />
          </Link>
