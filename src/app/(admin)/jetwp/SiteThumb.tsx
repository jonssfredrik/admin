...
export function SiteThumb({ screenshot, domain, size = "sm", className }: Props) {
  ...
  return (
    <div
      className={clsx(
        "shrink-0 overflow-hidden rounded-md border bg-surface shadow-sm",
        s.box,
        className,
      )}
      aria-label={`Thumbnail för ${domain}`}
    >
      <div className={clsx("flex items-center border-b bg-bg/60", s.chrome, s.pad)}>
        <span className={clsx("rounded-full bg-red-400/70", s.dot)} />
        <span className={clsx("rounded-full bg-amber-400/70", s.dot)} />
        <span className={clsx("rounded-full bg-emerald-400/70", s.dot)} />
      </div>
      <div className="relative h-full w-full" style={{ background: screenshot }}>
        <div className={clsx("flex h-full flex-col justify-end", s.content)}>
