...
const services: Service[] = [
  { name: "Nginx", description: "Web server & reverse proxy", status: "operational", latency: "8 ms", icon: Server },
  { name: "MariaDB", description: "Primär databaskluster", status: "operational", latency: "12 ms", icon: Database },
  { name: "Redis", description: "Object & page cache", status: "operational", latency: "2 ms", icon: Activity },
  { name: "PHP-FPM", description: "PHP 8.1 / 8.2 / 8.3 pooler", status: "degraded", latency: "184 ms", icon: Cpu },
  { name: "Cron scheduler", description: "WP-Cron & systemjobb", status: "operational", latency: "—", icon: Clock },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          hint={activeIncidents > 0 ? "kräver uppmärksamhet" : "allt lugnt"}
        />
      </div>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="flex items-center justify-between border-b px-5 py-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted">{nodes.length} noder · {nodes.reduce((s, n) => s + n.sites, 0)} sajter totalt</p>
          </div>
          <span className="text-xs text-muted">Uppdaterad för 12 sek sedan</span>
        </div>
        <div className="divide-y divide-border/60">
          {nodes.map((n) => {
            const cfg = nodeStatusCfg[n.status];
            return (
              <div key={n.id} className="grid grid-cols-1 gap-4 px-5 py-4 md:grid-cols-12 md:items-center">
                <div className="md:col-span-3">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs text-muted">
                    <span>{n.region}</span>
                    <span>·</span>
                    <span>{n.sites} sajter</span>
                    <span>·</span>
                    <span className="tabular-nums">up {n.uptime}</span>
                  </div>
                </div>
                <div className="grid grid-cols-4 gap-4 md:col-span-8">
                  <ResourceBar icon={Cpu} label="CPU" value={n.cpu} />
                  <ResourceBar icon={MemoryStick} label="RAM" value={n.memory} />
                  <ResourceBar icon={HardDrive} label="Disk" value={n.disk} />
                  <ResourceBar icon={Network} label="Nät" value={n.network} />
                </div>
                <div className={clsx("text-right text-xs font-medium md:col-span-1", cfg.text)}>
                  {cfg.label}
                </div>
              </div>
            );
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          <AreaChart data={cpuHistory} height={180} formatValue={(v) => `${v}%`} />
        </Card>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {services.map((s) => {
              const cfg = serviceCfg[s.status];
              const Icon = s.icon;
              return (
                <div key={s.name} className="flex items-center gap-3 rounded-lg border bg-bg px-3 py-2.5">
                  <Icon size={16} className="shrink-0 text-muted" />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="truncate text-sm font-medium">{s.name}</span>
                      <span className={clsx("text-[10px] font-semibold uppercase tracking-wider", cfg.tone)}>
                        {cfg.label}
                      </span>
                    </div>
                    <div className="truncate text-xs text-muted">{s.description}</div>
                  </div>
                  <span className="shrink-0 font-mono text-[11px] tabular-nums text-muted">{s.latency}</span>
                </div>
              );
            })}
          </div>
            ...
            {sites.map((s) => {
              ...
              return (
                <Link
                  key={s.id}
                  href={`/jetwp/${s.id}`}
                  className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm transition-colors hover:bg-bg"
                >
                  ...
                  <span className="shrink-0 font-mono text-[11px] tabular-nums text-muted">
                    {s.visits30d > 1000 ? `${Math.round(s.visits30d / 1000)}k` : s.visits30d}
                  </span>
                </Link>
              );
            })}
      ...
      <Card className="p-0 overflow-hidden">
        <div className="flex items-center justify-between border-b px-5 py-3">
          ...
          <Link href="/jetwp/activity" className="text-xs text-muted hover:text-fg">
            Visa alla →
          </Link>
        </div>
        <div className="divide-y divide-border/60">
              ...
              <div key={inc.id} className="flex items-start gap-3 px-5 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-2 text-xs text-muted">
                    <span className="font-mono">{inc.node}</span>
                    <span>·</span>
                    <span>{inc.time}</span>
                    <span>·</span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-[11px]">
        ...
        <span
          className={clsx(
            ...
            hot ? "text-amber-600 dark:text-amber-400" : "text-fg",
          )}
        >
          {value}%
        </span>
      ...
      <div className="mt-1 h-1 overflow-hidden rounded-full bg-fg/5">
        <div
          className={clsx("h-full rounded-full", hot ? "bg-amber-500" : warm ? "bg-fg/80" : "bg-fg/60")}
          style={{ width: `${value}%` }}
        />
      </div>
    </div>
